...
email_client = Client("u3mra51amezcopes8nx5csnkm2xn11", api_token="acqnqxckdz3hhaqkn1gebpj2vq9hcb")
    ...
    with get_db_session() as session:
        for name in confirmed_detections:
            ...
            if not recent_detection:
                ...
                email_image = buffer.tobytes()
                detection_time = datetime.now().strftime("%H:%M PST")

                email_client.send_message(f'{name} has been detected at {detection_time}', attachment=email_image, title="Hodor detected someone a the door!")
